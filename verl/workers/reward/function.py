            ...
            score = self.reward_fn(
                {
                    ...
                    "answer": data.non_tensor_batch["answer"][i],
                    "prompt": data.non_tensor_batch["prompt"][i],
                }
            )
            # Convert numpy values to Python floats to avoid type mismatch
            overall_score = float(score["overall"])
            reward_tensor[i, cur_response_length - 1] = overall_score
            for key, value in score.items():
                # Convert numpy values to Python floats for consistency
