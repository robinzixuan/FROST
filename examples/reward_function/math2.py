...
def length_reward(response: str, answer: str, tokenizer, **kwargs):
    ...
    if num_tokens <= budget:
        ...
        penalty = (num_tokens - 10 * budget) * 0.01
        reward = max(-1.0, -1 * penalty)
    else:
        penalty = min(0.9, (num_tokens - budget) * 0.01)
...
def attention_reward(response: str, prompt: str, tokenizer, model, **kwargs):
    ...
    if think_token_pos is None:
        # No </think> token found, use first token of last sentence as fallback
        sentences, sentence_boundaries = split_prediction_into_sentences(response)
        if sentences:
            # Get the last sentence and find its first token position
            last_sentence = sentences[-1]
            last_sentence_start_char = sentence_boundaries[-1][0]
            
            # Find the token position for the start of the last sentence
            text_up_to_last_sentence_start = prompt + ' ' + response[:last_sentence_start_char]
            tokens_up_to_last_sentence_start = tokenizer.tokenize(text_up_to_last_sentence_start)
            think_token_pos = len(tokens_up_to_last_sentence_start)
        else:
            # No sentences found, use the first token of the response
            think_token_pos = 0
...
def batch_attention_reward(responses: list[str], prompts: list[str], tokenizer, model, batch_size: int = 128, **kwargs):
    """Batch version of attention_reward function that processes multiple samples at once.
    
    Args:
        responses: List of response strings
        prompts: List of prompt strings
        tokenizer: The tokenizer to use
        model: The model to extract attention weights from
        batch_size: Number of samples to process in each batch to avoid OOM issues
        **kwargs: Additional arguments
    
    Returns:
        list: List of attention reward scores for each sample
    """
    ...
    # Process data in smaller batches to avoid OOM issues
    for i in range(0, len(responses), batch_size):
        batch_end = min(i + batch_size, len(responses))
        batch_responses = responses[i:batch_end]
        batch_prompts = prompts[i:batch_end]
        
        print(f"Processing batch {i//batch_size + 1}/{(len(responses) + batch_size - 1)//batch_size} "
              f"(samples {i+1}-{batch_end})")
        
        # Combine texts for this batch
        combined_texts = [prompt + ' ' + response for prompt, response in zip(batch_prompts, batch_responses)]
        
        # Calculate attention matrices for this batch
        batch_attention_matrices = batch_calculate_attention_matrix(combined_texts, tokenizer, model)
        
        for j, (response, prompt, attention_matrix) in enumerate(zip(batch_responses, batch_prompts, batch_attention_matrices)):
            sample_idx = i + j
            # Count tokens using BPE tokenization
            tokens = tokenizer.encode(response, add_special_tokens=False)
            ...
            print(f"Sample {sample_idx}: Attention matrix shape: {attention_matrix.shape}")
            ...
            for k, (start_token, end_token) in enumerate(sentence_token_boundaries):
                if start_token < len(attention_matrix) and end_token <= len(attention_matrix) and think_token_pos < len(attention_matrix):
                    sentence_attention_weights = attention_matrix[think_token_pos, start_token:end_token]
                    sentence_attention_sum = np.mean(sentence_attention_weights)
                    sentence_attention_sums.append(sentence_attention_sum)
            ...
            attention_scores.append(reward)
    
    return attention_scores


def batch_calculate_attention_matrix(contents: list[str], tokenizer, model, max_length=2048):
    """
    Calculate attention matrices for multiple contents in batch using the model's attention weights.
    ...
    Returns:
        list: List of attention matrices (2D numpy arrays) for each content
    """
    ...
    # Extract attention weights for the entire batch
    with torch.no_grad():
        # Ensure caching is disabled for gradient checkpointing compatibility
        if hasattr(model, 'config'):
            original_use_cache = model.config.use_cache
    ...
    layer_attention = attention_weights[target_layer]  # [batch_size, num_heads, seq_len, seq_len]
    
    # Check if the target head exists
    if target_head >= layer_attention.shape[1]:
        print(f"Warning: Target head {target_head} not found. Using last available head.")
        target_head = layer_attention.shape[1] - 1
    
    # Extract attention from specific layer and head for each sample in batch
    batch_attention_matrices = []
    for i in range(layer_attention.shape[0]):
        attention_matrix = layer_attention[i, target_head].cpu().numpy()  # [seq_len, seq_len]
        batch_attention_matrices.append(attention_matrix)
    
    print(f"Using layer {target_layer}, head {target_head}")
    print(f"Batch size: {len(batch_attention_matrices)}")
    print(f"Attention matrix shapes: {[matrix.shape for matrix in batch_attention_matrices]}")
    
    return batch_attention_matrices
...
def compute_score(
    ...
    # Extract all inputs for batch processing
    responses = []
    prompts = []
    ground_truths = []
    answers = []
    ...
    for reward_input in reward_inputs:
        ...
        prompts.append(reward_input["prompt"])
        ground_truths.append(reward_input["ground_truth"])
        answers.append(reward_input["answer"])
    ...
    # Calculate attention scores using batch processing if enabled
    if use_batch_attention and len(responses) > 1:
        print(f"Using batch attention processing for {len(responses)} samples")
        # Use the specified batch size to avoid OOM issues
        batch_size = min(attention_batch_size, len(responses))  # Don't exceed the number of samples
        attention_scores = batch_attention_reward(responses, prompts, tokenizer, model, batch_size=batch_size)
    else:
        print("Using individual attention processing")
        attention_scores = [attention_reward(response, prompt, tokenizer, model) for response, prompt in zip(responses, prompts)]
    ...
    for i in range(len(responses)):
        scores.append(
            {
                "overall": (1 - format_weight - length_weight) * accuracy_scores[i] + format_weight * format_scores[i] + length_weight * length_scores[i] + attention_weight * attention_scores[i],
