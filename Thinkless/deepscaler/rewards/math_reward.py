...
from deepscaler.globals import THOUGHT_DELIMITER_START, THOUGHT_DELIMITER_END, SHORT_DELIMITER_START, OAI_RM_MODEL
...
class RewardMathFn(RewardFn):
    ...
    def __call__(self, input: RewardInput) -> RewardOutput:
        ...
        elif SHORT_DELIMITER_START in model_response:
            model_solution = model_response.split(SHORT_DELIMITER_START)[1]
        else:
        ...
        for ground_truth in processed_ground_truths:
            ...
            if is_correct:
                if "<short>" in model_response:
                    return RewardOutput(reward=self.config.correct_short_reward, is_correct=True)
                elif "<think>" in model_response:
                    return RewardOutput(reward=self.config.correct_think_reward, is_correct=True)
            else:
                if "<short>" in model_response:
                    return RewardOutput(reward=self.config.wrong_short_reward, is_correct=False)
                elif "<think>" in model_response:
                    return RewardOutput(reward=self.config.wrong_think_reward, is_correct=False)
...
def deepscaler_reward_fn(solution_str: str, ground_truth: Union[str, List[str]], enable_llm = False, thinkless_rewards = {"correct_think_reward": 0.5, "correct_short_reward": 1.0, "wrong_think_reward": -1.0, "wrong_short_reward": -1.0}):
    # setup the reward function
    reward_config = RewardConfig()
    reward_config.correct_think_reward = thinkless_rewards["correct_think_reward"]
    reward_config.correct_short_reward = thinkless_rewards["correct_short_reward"]
    reward_config.wrong_think_reward = thinkless_rewards["wrong_think_reward"]
    reward_config.wrong_short_reward = thinkless_rewards["wrong_short_reward"]
