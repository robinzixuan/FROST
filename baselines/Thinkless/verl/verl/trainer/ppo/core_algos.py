...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange, thinkless_alpha=0.001):
    ...
    if thinkless_alpha is not None: # Decoupled GRPO
        ...
        avg_len = eos_mask.sum(dim=1).float().mean()

        cond_loss = verl_F.masked_mean(token_loss, cond_mask)
        resp_loss = verl_F.masked_mean(token_loss, resp_mask)
